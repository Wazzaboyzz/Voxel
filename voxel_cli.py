...
This is what "one command, one book ready" means in practice:
  ...
  Picture book (like Luna):
    ...
    Image generation for the 'book' command has two modes (Gemini's free
    tier 429s on every call without billing, which is off the table - see
    HANDOFF.md Phase 8c):
      --manual-images   writes image_prompts.md (no API calls, no images
                        generated) so you can generate each one by hand in
                        VEO3, Google Flow, etc.
      --images-dir PATH  uses images you already generated by hand and
                        uploaded to PATH, instead of calling Gemini.
    Without either flag, it still tries Gemini (kept for completeness /
    future free-source swap), and will still 429 today.
...
def cmd_book(args):
    ...
    if args.manual_images:
        ...
        print("[voxel] Generating illustrations (Gemini - requires GEMINI_API_KEY + billing; see --manual-images instead)...")
